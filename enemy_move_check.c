...
/*   Updated: 2022/03/19 17:44:00 by fdrudi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_enemy_help(t_env *e, int i, int j, int a)
{
	if (a < 0)
	{
		if (i < 0 && ft_check_cond(e, e->n.n_y[j], e->n.n_x[j] - 1, j) == 0)
			e->n.x_m[j] = -1;
		else if (i > 0 && ft_check_cond(e, e->n.n_y[j], e->n.n_x[j] + 1, j) == 0)
			e->n.x_m[j] = 1;
		else
			e->n.x_m[j] = 0;
		e->n.y_m[j] = 0;
	...
	{
		if (i < 0 && ft_check_cond(e, e->n.n_y[j] - 1, e->n.n_x[j], j) == 0)
			e->n.y_m[j] = -1;
		else if (i > 0 && ft_check_cond(e, e->n.n_y[j] + 1, e->n.n_x[j], j) == 0)
			e->n.y_m[j] = 1;
		else
			e->n.y_m[j] = 0;
		e->n.x_m[j] = 0;
...
int	ft_check_enemy_help_b(t_env *e, int j, int a)
{
	...
	{
		if (ft_check_cond(e, e->n.n_y[j], e->n.n_x[j] - 1, j) == 0)
			e->n.x_m[j] = -1;
